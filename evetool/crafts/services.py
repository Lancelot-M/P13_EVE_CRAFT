from crafts.models import Item, Blueprint, InputProduction, Invention, InputInvention
from django.core.exceptions import ObjectDoesNotExist
from crafts.calculator import Calculator
from config import PROTECTIVE_COMPONENTS, FROM_REACTION, TRIGLAVAN_COMPONENTS, CAPITAUX,\
    SUBCAP, DECRYPTORS_LIST
...
class Services():
    ...
    def read_components(self):
        ...
        for component in self.data["composant"]:
            if component == "Protective":
                ...
                for item in items:
                    if item.name in PROTECTIVE_COMPONENTS or item.name \
                            in TRIGLAVAN_COMPONENTS:
                        continue
                    else:
                        self.get_stats(item.name)
    ...
    def item_data(self, bp):
        ...
        if self.data["arborescence"] == '0':
            ...
            while self.check_material_only(data) is False:
    ...
    def prod_data(self, bp):
        ...
        for item in bp.items_needed.all():
            if bp.tech_2:
                ...
                data[item.name] = (composants.quantity * float(0.98) *
                                   float(self.data["item_t2_structures"]) *
                                   float(self.data["item_t2_rigg"])) / bp.quantity_produced
            elif bp.reaction:
                composants = InputProduction.objects.get(items=item, blueprints=bp)
                data[item.name] = (composants.quantity *
                                   float(self.data["reaction_structures"]) *
                                   float(self.data["reaction_rigg"])) / bp.quantity_produced
            else:
                composants = InputProduction.objects.get(items=item, blueprints=bp)
                data[item.name] = ((composants.quantity * float(self.data["ME"]) *
                                    float(self.data["item_t1_structures"]) *
                                    float(self.data["item_t1_rigg"])) / bp.quantity_produced)
        return data
    ...
    def check_material_only(self, compo):
        ...
        for key in compo.keys():
            ...
            if item.group_belong.name == "Fuel Block":
                if self.data["fuel_block"] is False:
                    continue
        ...
        for key, value in compo.items():
            try:
                ...
                if item.week0_value is None:
                    ...
                    input_value += float(item.week0_value) * value
            except ObjectDoesNotExist:
                print("ERROR WITH DECRYPTOR FOR", key)
        bp_invention = Invention.objects.get(output_blueprint=bp)
        chance = float(bp_invention.succes_rate) * float(1 + ((
                       int(self.data["science1"]) + int(self.data["science2"])) / 30) +
                       int(self.data['encryption']) / 40)
    ...
    def without_decryptor(self, bp_data):
        ...
        for datacore in bp_data["datacores"]:
            no_decryptor_compo[datacore.items.name] = ((datacore.quantity
                                                       / bp_data["chance"])
                                                       / bp_data["blueprint"].quantity_produced)
            try:
                item = Item.objects.get(name=datacore.items.name)
                no_decryptor_total += float(item.week0_value) * datacore.quantity
            except ObjectDoesNotExist:
    ...
    def decryptor_bonus(self, bp_data, decryptor_data):
        ...
        for core in bp_data["datacores"]:
            data[core.items.name] = (((core.quantity / bp_data["chance"]
                                     * float(decryptor_data["Probability"]))
                                     / bp_data["blueprint"].quantity_produced)
                                     / (1 + decryptor_data["Add Run"]))
        for key, value in data.items():
            try:
                item = Item.objects.get(name=key)
                total += float(item.week0_value) * value
            except ObjectDoesNotExist:
                print("ERROR WITH DECRYPTOR FOR", item.name)
        total += ((bp_data["input_value"]
                  * (100 - decryptor_data["Add ME"]) / 100)
                  + float(decryptor.week0_value))
        data[decryptor.name] = ((1 / (bp_data["chance"] * float(decryptor_data["Probability"])))
                                / bp_data["blueprint"].quantity_produced
                                / (1 + decryptor_data["Add Run"]))
        for key, value in bp_data["composants"].items():
            data[key] = value * float((100 - decryptor_data["Add ME"]) / 100)
