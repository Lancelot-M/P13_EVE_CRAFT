...
class CreateDb():
    ...
    def check_invention_tags(self, invention_data):
        """read blueprint's invention tags"""
        for tag in ["materials", "products", "time"]:
                            ...
                            if key2 == "invention":
                                for product in value2["products"]:
                                    try:
                                        ...
                                        try:
                                            invention_data = Invention.objects.get(input_blueprint=blueprint,
                                                                                   output_blueprint=bp_t2)
                                        except ObjectDoesNotExist:
                                            invention_data = Invention(input_blueprint=blueprint,
                                                                       output_blueprint=bp_t2,
                                                                       time=value2["time"],
                                                                       run_per_success=product["quantity"],
                                                                       succes_rate=product["probability"])
                                            invention_data.save()
                                            for material in value2["materials"]:
                                                try:
                                                    datacore = Item.objects.get(types_id=material["typeID"])
                                                    research = InputInvention(items=datacore,
                                                                              inventions=invention_data,
                                                                              quantity=material["quantity"])
                                                    research.save()
                                                except ObjectDoesNotExist:
